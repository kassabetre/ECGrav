...
> **Status:** 1.11.0. Actively developed research code. See [ROADMAP.md](ROADMAP.md)
...
```wolfram
PacletInstall["/path/to/ECGrav/build/ECGrav-1.11.0.paclet", ForceVersionInstall -> True];
Needs["ECGrav`"];
```
...
current release (1.11.0). Earlier releases are preserved as git tags:

- `v1.11.0` — current: the constant-thermodynamic-length metric is rebuilt from one
