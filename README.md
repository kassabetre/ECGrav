...
> **Status:** 1.9.0. Actively developed research code. See [ROADMAP.md](ROADMAP.md)
...
```wolfram
PacletInstall["/path/to/ECGrav/build/ECGrav-1.9.0.paclet", ForceVersionInstall -> True];
Needs["ECGrav`"];
```
...
current release (1.9.0). Earlier releases are preserved as git tags:

- `v1.9.0` — current: the graph correlation time caught up with its complex-space
