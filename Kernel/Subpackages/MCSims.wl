...
minE = curE;
excitedEnergy=N[minE+SetPrecision[1000.0,precision]*minE,precision];
minStates={Amcur};
excitedStates={Amcur};
...
step[bta_]:=Block[{delE,expdelE,flipSpin,accept},
...
step[bta_Real]:=Block[{delE,expdelE,flipSpin,accept},
...
(*If[Mod[numsweeps,1]\[Equal]0,ParallelSow[AdjacencyGraph[Amat,ImageSize\[Rule]{140,140}]]
];*)
...
ECGrav`GraphMultiHistogram[seedGraph_List,betaLow_Real,betaHigh_Real,hamiltonian_,delH_,obs_,NN_Integer,UnlabeledVerticesYes_Integer]:=
(*************************************)
...
(*computeBFs[dat_Association]:=Module[{output,numTemps=Length[dat],betas=Keys[dat],
ntab=Table[Length[dat[[Key[i]]]],{i,Keys[dat]}],logntab,curFs,nextFs,iterNum,del,deltab},
logntab=Log[ntab*1.0];
...
Print[" Computing -beta*freeenergy for betas ",betas, " on data ",dat];
...
,{k,1,numTemps},DistributedContexts -> {"ECGrav`MCSims`"}];
...
(* ::Item:: *)
(*GraphParallelTempering Overload*)
