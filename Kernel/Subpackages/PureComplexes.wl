...
ComplexAutomorphismGenerators::argerr="Internal helper: input should be a list of facets of the form ComplexAutomorphismGenerators[facetsLst_List].";
...
(*   (* Note:  *) *)
(****************************************)
(*Given a general simplicial complex (not necessarily clique complex and not necessarily pure) that may or may not be connected, it computes the order of the automorphism group. It reads the order off the automorphism group of the vertex-facet incidence graph built by ComplexAutomorphismIncidenceGraph, which is indifferent to facet size and so handles mixed-size facets correctly.*)
(*The previous implementation reduced each facet to a single leaf and took the automorphism group of the reduced clique graph. That reduction discards facet SIZES, so a reduced facet whose parent had 3 vertices could map onto one whose parent had 2: {{1,2,3},{1,4}} reduces to the path 2-1-4, whose endpoint swap is not an automorphism of the complex. It guarded against this inside isPermutationOfFacets, but only when the reduced graph had maximal cliques that were not facets -- which is not the case here -- so mixed-size complexes came back with the group too large. Equal-size facets were never affected, which is why this only ever showed up on non-pure input.*)
*)
If[facetsLst==={},1,
...
*)
Module[{canonicalGraph,relabelingRule},

Catch[canonicalGraph=CanonicalGraph[GraphFromCliques[facetsLst]];

relabelingRule=Normal[FindGraphIsomorphism[GraphFromCliques[facetsLst],canonicalGraph][[1]]];

If[Length[facetsLst]==1,Throw[{SymmetricGroup[Length[facetsLst[[1]]]],Reverse/@relabelingRule}, "ECGravReturn$45"]];

(*The canonical relabeling sends the vertices to 1..n, so the vertex action returned by
	ComplexAutomorphismGenerators is already expressed in the canonical labels the caller is told
	about. Reading generators off the incidence graph also removes the old filtering step: that
	existed only because the automorphism group was taken from the clique-graph completion, whose
	maximal cliques need not be facets. The incidence graph has no such discrepancy to repair.*)
{PermutationGroup[ComplexAutomorphismGenerators[Sort/@(facetsLst/.relabelingRule)][["vertex"]]],
	(Reverse/@relabelingRule)}, "ECGravReturn$45"]
...
(* ::Item::Closed:: *)
(*ComplexAutomorphismGenerators*)


(* :Code Section *)

(* Private helper *)
ComplexAutomorphismGenerators[facetsLst_List]:=
(*
(****************************************)
(*   (* Last updated 08/11/2026. *) *)
(*   (* Version: 1 *) *)
(****************************************)
(*Generators of the automorphism group of the complex in both of its natural actions, from one
	call to GraphAutomorphismGroup on the incidence graph.

	Those generators preserve the two sides of the bipartition, so restricted to the vertex block
	they generate the action on vertices, and restricted to the facet block they generate the
	image in the symmetric group on the facets -- the facet automorphism group. The image of a
	group under a homomorphism is generated by the images of the generators, so neither action
	requires the group to be enumerated.

	Returning generators instead of elements is not merely faster, it is often the only
	possibility: for a 20-facet complex whose facets are pairwise interchangeable the facet group
	is the full symmetric group on 20 points, of order 2432902008176640000, which cannot be
	listed at all.

	FACET NUMBERING: facet j means facetsLst[[j]] -- the position in the list AS SUPPLIED, not the
	lexicographic rank of the facet. Given {{1,2,4},{1,2,3}} facet 1 is {1,2,4}, while given
	{{1,2,3},{1,2,4}} facet 1 is {1,2,3}. The permutations in the facet action therefore index the
	caller's own list directly, with no reordering to undo.

	VERTEX NUMBERING: vertex i means (Union@@facetsLst)[[i]], the i-th distinct vertex label in
	ascending order. The two coincide whenever the complex is labelled 1..n, which is what
	PureComplexAutomorphismGroup arranges by relabeling canonically before calling this.

	Generators lying in the facet stabilizer act trivially on the facets and collapse to the
	identity there; they are dropped, and if none survives the trivial group's generator is
	returned so the result is still a usable PermutationGroup argument.*)
*)
Module[{vertexLabels,vCount,fCount,graphGenerators,restrict,vertexGenerators,facetGenerators},

	vertexLabels=Union@@facetsLst;
	vCount=Length[vertexLabels];
	fCount=Length[facetsLst];

	graphGenerators=GroupGenerators[GraphAutomorphismGroup[
		ComplexAutomorphismIncidenceGraph[facetsLst]]];

	(*restrict a permutation of the incidence graph's nodes to one block of consecutive nodes*)
	restrict[perm_,offset_,count_]:=
		PermutationCycles[Table[PermutationReplace[offset+k,perm]-offset,{k,count}]];

	vertexGenerators=DeleteCases[restrict[#,0,vCount]&/@graphGenerators,Cycles[{}]];
	facetGenerators=DeleteCases[restrict[#,vCount,fCount]&/@graphGenerators,Cycles[{}]];

	<|"vertex"->If[vertexGenerators==={},{Cycles[{}]},vertexGenerators],
	  "facet"->If[facetGenerators==={},{Cycles[{}]},facetGenerators]|>
];

(* Catch-all Pattern *)
ComplexAutomorphismGenerators[args___]:=(Message[ComplexAutomorphismGenerators::argerr, args];
$Failed);
...
of facets *)
*)
(*The facet automorphism group is the image of the automorphism group in the symmetric group on
	the facets, which is generated by the images of any generating set -- no enumeration, and no
	canonical relabeling, since the facets are indexed by their position in the input list.

	FACET NUMBERING: the returned permutations act on 1..Length[facetsLst], where j is
	facetsLst[[j]] as supplied. Lexicographic order plays no part: for {{1,2,3},{1,2,4}} facet 1
	is {1,2,3}, but for {{1,2,4},{1,2,3}} facet 1 is {1,2,4}.*)
PermutationGroup[ComplexAutomorphismGenerators[facetsLst][["facet"]]];
...
*)
(*The facet automorphism group is the image of the automorphism group in the symmetric group on
	the facets, generated by the images of any generating set. For a clique complex the complex
	and its 1-skeleton have the same automorphisms, so nothing here needs the clique graph.

	FACET NUMBERING: the returned permutations act on 1..Length[facetsLst], where j is
	facetsLst[[j]] as supplied, not the lexicographic rank of the facet.*)
PermutationGroup[ComplexAutomorphismGenerators[facetsLst][["facet"]]];
